...
function spawn(commit, cmd, callback) {
  ...
  commit.spawn(cmd, options).on('exit', function(code, signal) {
...
module.exports = function listen(port, callback) {
  ...
  git.on('commit', function(commit) {
    async.series([
      ...
      if (err) {
        // ... can I remove the commit?  not much else to do, would be nice if
        // git push could be failed, but I think its too late for that.
      }
    });
  });
