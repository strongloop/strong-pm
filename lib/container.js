...
function Container(options) {
  ...
  this._svcDir = path.resolve(this.baseDir, 'svc', String(this.svcId));
...
Container.prototype.remove = function(callback) {
  ...
  self.debug('removing %j %j', self.svcId, self._svcDir);

  rmrf(self._svcDir, function(err) {
    self.debug('removed %j %j: err? %j', self.svcId, self._svcDir, err);
    assert.ifError(err);
    return callback();
  });
};
