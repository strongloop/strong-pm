...
prototype.initOrUpdateDb = function(meshApp, callback) {
  ...
  function stopProcess(proc, callback) {
    ...
    proc.stopReason = g.t('StrongLoop Process Manager was stopped');
    proc.stopTime = g.d(new Date());
    proc.save(callback);
  }
};
...
prototype.onDeployment = function(service, req, res) {
  ...
  service.instances(function(err, instances) {
    if (err) {
      debug(err);
      res.status(500).set({});
      res.end(g.t('Error while deploying: %s', err.message));
      return;
    }
    if (instances.length <= 0) {
      debug(err);
      res.status(500).set({});
      res.end(g.t('Error while deploying: no instance found'));
      return;
    }
...
prototype.getInstanceEnv = function(instanceId, callback) {
  ...
  Instance.findById(instanceId, function(err, instance) {
    ...
    if (!instance)
      return callback(
        g.Error('Unable to find instance with id %s', instanceId));

    instance.serverService(function(err, service) {
      if (err) return callback(err);
      if (!service)
        return callback(
          g.Error('Unable to find service for instance with id %s', instanceId)
        );
