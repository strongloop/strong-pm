...
PackReceiver.prototype.processTarball = function(req, res, tarGzPath, hash) {
  ...
  readStream.on('error', function(err) {
    self.sendError(res, g.t('Read %s failed with: %s', tarGzPath, err));
  });
  unzipStream.on('error', function(err) {
    self.sendError(res, g.t('Untar %s failed with: %s', tarGzPath, err));
  });
  ...
  untarStream.on('end', function() {
    ...
    res.writeHead(200);
    res.end(g.t('Application received'));
  });
};
...
PackReceiver.prototype.getTarballHash = function(req, res, tarballPath) {
  ...
  tarballStream.on('error', function(err) {
    g.log('Read %s for sha failed: %s', tarballPath, err);
    self.sendError(res, g.t('Read %s for sha failed: %s', tarballPath, err));
  });
