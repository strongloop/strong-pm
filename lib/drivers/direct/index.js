...
function Driver(options) {
  ...
  if (this._app) {
    this._wsRouter = new WebsocketRouter(this._app, 'supervisor-control');
  }
}
...
Driver.prototype._containerById = function(instanceId) {
  ...
  if (!container) {
    ...
    if (this._app) {
      var port = mandatory(this._server.port());
      var channel = this._wsRouter.createChannel(onRequest);
      var request = channel.request.bind(channel);
      var token = channel.getToken();
      var control = util.format('ws://%s@127.0.0.1:%d', token, port);

      debug('container control socket: %s', control);

      // Patch our request method in... because the container calls request
      // itself (which should be changed sometime).
