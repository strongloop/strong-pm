...
DockerDriver.prototype.requestOfInstance = function(id, req, cb) {
  ...
  if (instance.current) {
    ...
    return instance.request ? instance.request(req, cb)
                            : instance.current.request(req, cb);
  } else if (cb) {
...
DockerDriver.prototype.start = function(instanceMetas, cb) {
  ...
  async.parallel([
    ...
    if (err) {
      self.error('Error in Docker Driver startup:', err);
      return cb(err);
    }
...
DockerDriver.prototype._instance = function(id) {
  ...
  if (!instance) {
    ...
    if (this._wsRouter) {
      var port = mandatory(this.server.port());
      var channel = this._wsRouter.createChannel(onInstanceRequest);
      var request = channel.request.bind(channel);
      var token = channel.getToken();
      var addr = publicIp();
      var wsPath = this._wsRouter.path;
      var control = fmt('ws://%s@%s:%d%s', token, addr, port, wsPath);

      debug('container control socket: %s', control);

      instance.request = request;
      instance.channel = channel;
      instance.startOpts.control = control;
    }
    this._instances[id] = instance;
  }
  return this._instances[id];

  function onInstanceRequest(req) {
