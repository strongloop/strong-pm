...
DockerDriver.prototype._resume = function(id, meta) {
  ...
  image.on('error', function(err) {
    self.error(g.t('error deploying %s %j:', id, err));
    throw err;
  });
  image.on('image', function() {
    debug('Deployment image for %s %j:', id, image.name);
    self.startInstance(id, function(err) {
      if (err) {
        self.error(g.t('error resuming %s %j', id, err));
      } else {
        self.log(g.t('resumed instance %s', id));
      }
    });
  });
};
...
DockerDriver.prototype.stop = function(cb) {
  var self = this;
  this.log(g.t('shutting down driver'));
