...
Server.prototype._onMetrics = function _onMetrics(req, callback) {
  ...
  function saveMetric(wid, asyncCb) {
    ...
    var q = {where: {serviceInstanceId: self.instanceId, workerId: +wid}};
...
Server.prototype._onRunnerRequest = function _onRunnerRequest(req, callback) {
  ...
  var self = this;
  function onComplete() {
    self.emit('notification', req.cmd);
    if (callback) callback();
  }
  ...
  switch (req.cmd) {
    case 'started':
      return this._onMasterStart(req, onComplete);
    case 'fork':
      return this._onWorkerStart(req, onComplete);
    case 'exit':
      return this._onWorkerExit(req, onComplete);
    case 'status':
      // Unused
      return onComplete();
    case 'listening':
      return this._onWorkerListening(req, onComplete);
    case 'metrics': {
      return this._onMetrics(req, onComplete);
    }
    case 'object-tracking':
    case 'cpu-profiling':
    case 'heap-snapshot':
      return this._onProfileStatusUpdate(req, onComplete);
    default: {
...
Server.prototype._updateCommit = function _updateCommit(commit, callback) {
  ...
  async.waterfall([
    ServerService.findOne.bind(ServerService),
    updateAndSave
  ], function(err){
    assert.ifError(err);
    if (callback) callback(err);
...
Server.prototype._loadModels = function _loadModels(callback) {
  ...
  debug('Initial setup. ServiceID: %d, GroupId: %d, ExecId: %d, InstId: %d',
    this.serviceId, groupId, this.executorId, this.instanceId
  );
  ...
  var instance = new Instance({
    ...
    serverServiceId: this.serviceId,
    groupId: groupId,
    cpus: 'CPU',
  });
