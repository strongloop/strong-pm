...
function Server(originalCommand, baseDir, listenPort, controlPath) {
  ...
  // setup deployment endpoints and receiver
  this._receiver = setupPushReceiver(this, this._baseApp, this._baseDir);
...
Server.prototype.onDeployment = function(service, req, res) {
  ...
  if (container) {
    ...
    container = this._containers[svcId] = new Container({
      console: console,
      serviceId: svcId,
      // XXX(sam) for logging purposes... need to push into runner, would be
      // nicer if we had a human name... do we?
      serviceName: fmt('svc-%s', svcId),
      start: this.getStartCommand(),
    });
    container.svcId = svcId;
  }

  this._receiver(svcId, container, req, res);
};

Server.prototype._onCommit = function(commit, container) {
  ...
  // XXX(sam) preparation should be part of the driver/container
