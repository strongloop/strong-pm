...
Server.prototype._onMetrics = function _onMetrics(req, callback) {
  ...
  function saveMetric(wid, asyncCb) {
    ...
    Process.findOne(q, function(err, proc) {
      ...
      m.serviceInstanceId = self.instanceId;
...
Server.prototype._updateCommit = function _updateCommit(commit, callback) {
  var Service = this._app.models.Service;
...
Server.prototype._updateInstance =
  function _updateInstance(status, newCommit, callback) {
    ...
    function updateCommitInfo(container, asyncCb) {
      container.currentDeploymentId = newCommit.hash;
      container.save(function(err) {
        asyncCb(err);
      });
    }

    function updateInstance(instance, asyncCb) {
      ...
      current.request(req, function(res) {
        if (res.error) return asyncCb(res.error);
        instance.npmModules = res;
        instance.save(asyncCb);
      });
    }

    tasks.push(m.ServiceContainer.findOne.bind(m.ServiceContainer));
    if (newCommit) tasks.push(updateCommitInfo);
    tasks.push(m.ServiceInstance.findOne.bind(m.ServiceInstance));
    tasks.push(updateInstance);
...
Server.prototype._loadModels = function _loadModels(callback) {
  ...
  var Service = this._app.models.Service;
  var Container = this._app.models.ServiceContainer;
  var Instance = this._app.models.ServiceInstance;

  this.serviceId = process.env.STRONGLOOP_SERVICE_ID || 1;
  var groupId = process.env.STRONGLOOP_SERVICE_GROUP_ID || 1;
  this.executorId = process.env.STRONGLOOP_EXECUTOR_ID || 1;
  this.containerId = process.env.STRONGLOOP_CONTAINER_ID || 1;
  // A container contains only one instance. In order to allow easy mapping
  // between container and instance, they share the same ID. 
  this.instanceId = process.env.STRONGLOOP_CONTAINER_ID || 1;

  debug('Initial setup. Service: %s, Group: %s, Exec: %s, Cont: %s, Inst: %s',
    this.serviceId, groupId, this.executorId, this.containerId, this.instanceId
  );
  ...
  var instance = new Instance({
    id: this.instanceId,
    serviceContainerId: this.containerId
  });
...
Server.prototype.updateEnv = function (env) {
  ...
  function updateService(cb) {
    Service.findOne(function(err, service){
      ...
      service.env = self._env.all();
      service.save(cb);
    });
  }

  function restartRunner(err) {
