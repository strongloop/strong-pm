...
Server.prototype._loadModels = function _loadModels(callback) {
  ...
  var container = new Container({
    ...
    applicationPort: this._env.get('PORT')
  });
...
Server.prototype.updateEnv = function (env) {
  ...
  function updateContainer(cb) {
    Container.findOne(function(err, container) {
      if (err) return cb(err);
      container.applicationPort = self._env.get('PORT');
      container.save(cb);
    });
  }
