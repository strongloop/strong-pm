...
// XXX(sam) @kraman, I think much of the code in here, specifically ones that
// interact with the mesh models, should be moved to service-manager, leaving
// server.js to just integrate the independent components. Thoughts?
...
function Server(cmdName, baseDir, listenPort, controlPath, enableTracing) {
  ...
  // The express app on which the rest of the middleware (mesh-models, deploy
  // receive hooks etc.) are mounted.
  ...
  var ctlOptions = {
    server: this,
    //driver: this._driver; // FIXME should not directly interact with driver
