...
function printHelp($0, prn) {
  var USAGE = g.t('sl-pm-install.txt')
                .replace(/%MAIN%/g, $0)
                .trim();
  prn(USAGE);
}
...
function install(argv, callback) {
  ...
  if (!(jobConfig.pmPort > 0)) {
    g.error('Port %j is not valid, try `{{%s --help}}`.',
                  jobConfig.pmPort, $0);
    return callback(g.Error('usage'));
  }

  if (!(jobConfig.basePort > 0)) {
    g.error('Base port %j is not valid, try `{{%s --help}}`.',
  ...
  if (!jobConfig.systemd &&
    ...
    g.error('Invalid usage (only {{upstart}} "0.6" and "1.4" supported)' +
                  ', see `{{%s --help}}`', $0);
    return callback(g.Error('usage'));
  }
  ...
  if (jobConfig.env.STRONGLOOP_PM_HTTP_AUTH &&
    ...
    g.error(
      'Bad {{http-auth}} specification: %s',
      jobConfig.env.STRONGLOOP_PM_HTTP_AUTH);
    return callback(g.Error('usage'));
  }

  if (!/^(direct|docker)$/.test(jobConfig.driver)) {
    g.error('Invalid driver: %s', jobConfig.driver);
    return callback(g.Error('usage'));
  }

  if (jobConfig.upstart === '0.6' && jobConfig.driver === 'docker') {
    g.error('{{Upstart}} 0.6 does not support {{setgid}}. Please run:\n' +
  ...
  function report(err) {
    if (err) {
      g.error('Error installing service \'%s\':',
...
function ensureDocker(options, callback) {
  if (options.driver === 'docker') {
    ...
    docker.info(function(err) {
      if (err) {
        g.error('Docker not usable: %s', err);
      }
      callback(err);
    });
  } else {
...
function writeSeedEnvironment(options, callback) {
  ...
  function writeEnv(env, path, cb) {
    ...
    if (options.dryRun) {
      g.log('Would seed environment with: %j', env);
      return cb();
    } else {
