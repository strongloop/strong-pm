...
function printHelp($0, prn) {
  ...
  prn('  -u,--user         User to run sl-pm as (default current user).');
  prn('  -p,--port PORT    Listen on PORT for git pushes (no default).');
  prn('  -n,--dry-run      Don\'t write any files.');
  prn('  -j,--job-file     Path of Upstart job to create (default /etc/init/strong-pm.conf)');
  prn('  -f,--force        Overwrite existing job file if present');
  prn('  --upstart         Specify the version of Upstart to target: "0.6" or "1.4" (default)');
}
