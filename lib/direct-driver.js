...
Driver.prototype.onServiceDestroy = function(svcId, callback) {
  var self = this;

  debug('destroy service %d', svcId);

  self.stopService(svcId, function() {
    self.removeService(svcId, callback);
  });
};

Driver.prototype.stopService = function(svcId, callback) {
  var container = this._containers[svcId];

  debug('stop service %d exists? %j', svcId, !!container);
