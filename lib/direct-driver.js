...
Driver.prototype.start = function(callback) {
  ...
  function run(svcId, callback) {
    ...
    this.containerById(svcId).runCurrent(callback);
  }
...
Driver.prototype.stop = function() {
  // XXX(sam) should probably take a callback, because container stop is async,
  // but the server doesn't care, ATM.
...
Driver.prototype.onDeployment = function(svcId, req, res) {
  this.containerById(svcId).onDeployment(req, res);
};

// XXX(sam) make method private?
Driver.prototype.containerById = function(svcId) {
  ...
  if (!container) {
    ...
    // XXX(sam) there is a case to be made that the requests should be handled
    // here, and we should call methods on server (like server.addNewProcess()).
    // However... a) I'm punting for now. b) the requests will be identical for
    // all THREE driver types (direct, docker, central), because all three will
    // be receiving the same request JSON protocol from a strong-supervisor.
    // How the supervisor is caused to run is different, but not its update
    // request protocol messages.
    container.on('request', this.emit.bind(this, container));
  }
